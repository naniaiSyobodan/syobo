...
my @FukusoList = ("服装:自由","活動服","乙種(法被) ","その他");		# 服装リスト
...
#第2階層のメニュー
my @SecondMenu = (	"href=\"https://kyum.nido.jp/syobo/okurenger.html\">長野市消防団 安否確認サービス" ,
					"href=\"http://nagano-bousai.jp/mail_service.html\">長野市防災メール登録" ,
					"href=\"https://www.shinshu-oenshop.net\">信州消防団員応援ショップ" ,
					"href=\"https://kyum.nido.jp/syobo/yousi.html\">各種用紙" ,
...
print "<br>NEWS update finish";
	...
	print qq|
		このページ<br><img src="img/qrcode_kyum_nido_jp.png"><br>
		syobo.naniai@gmail.com<br><a href="mailto:syobo.naniai@gmail.com"><img src="img/gmail.png"></a><br>
		石坂電話帳登録<br>
			<a href="mailto:mishizaka-0721kr\x40ezweb.ne.jp">
				<img src="img/ishizakaQR.png">
			</a>|;
}
print qq|<br>\n|;
print qq|
	...
	<a class="btn--orange" href="https://kyum.nido.jp/syobo/okurenger.html">長野市消防団 安否確認サービス</a>
...
sub	ShowGyoji
{
	...
	if($filecnt > 0) {
		$PdfFile = $filelist[0];
		#print qq|[$PdfFile]<br></a>|;
		my	@filename    = split(/\./, $PdfFile);
		my	$fileExt = $filename[@filename - 1];
		$fileExt =~ tr/A-Z/a-z/;
		
		print qq|<a class="btn--orange" href="$PdfFile">資料ファイル($fileExt)</a>　　\n|;
	}
	if($rdSiryo ne "") {
	...
	#	参加予定団員の編集
	#	https://kyum.nido.jp/syobo/sc.cgi?mode=SankaYoteiEdit&recno=353&hanno=1
	if($HanNo eq 0) {	#分団の設定ならば参加対象とする班を表示
		...
		if($findCnt eq 1) {
			#班単独の行事ならば、編集のリンクを表示  https://kyum.nido.jp/syobo/
...
sub	InputGyoji
{
	...
	print qq|<br>コメント<br><input type="text" name="rdComment" value="$rdMemo" style="width: 50em;"><br>|;
...
sub UploadPdfWrite{
	...
	my	$fileExt = $filename[@filename - 1];
	$fileExt =~ tr/A-Z/a-z/;
	my	$PathName = './doc/Sy' . $rdYear . $rdMonth . $rdDay . '.' . $fileExt;
    ...
    elsif($fileExt eq "pdf" or $fileExt eq "jpg" or $fileExt eq "jpeg" or $fileExt eq "gif" or $fileExt eq "png") {
